...
object NewGenSerialServerDispatcher {
  val Eof = Future.exception(new Exception("EOF") with NoStacktrace)
  // We don't use Future.never here, because object equality is important here
  val Idle = new NoFuture
  val Closed = new NoFuture
}
/**
 ...
 */
abstract class NewGenSerialServerDispatcher[Req, Rep, In, Out](trans: Transport[In, Out])
  extends Closable {
    ...
    /** Exposed for testing */
    def isClosing: Boolean = state.get() eq Closed
...
class SerialServerDispatcher[Req, Rep](
  ...
  extends NewGenSerialServerDispatcher[Req, Rep, Rep, Req](trans) {
    trans.onClose ensure {
      service.close()
    }
